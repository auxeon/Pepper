...
node** bst_insert(node** n, node** pn, int d){
    if(!*n){
        *n = bst_create_node(d);
        (*n)->parent = pn?*pn:NULL;
        return n;
...
node* bst_from_postorder(int* data, int l, int r){
    if(l>r){
        return NULL;
    }
...
void bst_log_node_details(node**n, node** k){
 ...
 * -1 not a kid
 */
int bst_is_k_left_or_right(node** n, node** k){
 ...
 */
node** bst_inorder_pred(node** n, int key){
    node** found = NULL;
...
*/
void bst_copy_node(node** d, node** s){
    (*d)->data = (*s)->data;
}
 ...
 */
void bst_print_postorder(node** n){
    if(!(*n)){
        return;
    }
