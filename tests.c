...
void data_callback_test6(ma_device* pDevice, void* output, const void* input, ma_uint32 frame_count){
    ...
    ma_decoder_read_pcm_frames(pDecoder, output, frame_count, &frames_read);
...
void test6(){
    ...
    const char* buffer = "resources/ffdp_wrong_side_of_heaven_cover_abhikalp_unakal.mp3";
...
void test7() {
    ...
    const char* buffer[] = { "resources/ffdp_wrong_side_of_heaven_cover_abhikalp_unakal.mp3", "resources/daft_punk_get_lucky.mp3" };
...
void test10() {
   if (FPS){
    ...
    FILE* f = fopen("resources/boxconfs.dat","r");
    int nboxes;
